...
class HarmonyStreamState:
    ...
    def __init__(self,
        ...
        # Track first delta for role handling
    ...
    def _create_delta_from_parser_state(self) -> Optional[Dict[str, Any]]:
        """Create OpenAI delta from current parser state."""
        if not self.parser.last_content_delta:
            return None

        logger.debug(
            "Request %s: creating delta - channel=%s, recipient=%s, content_delta='%s'",
            self.request_id, self.parser.current_channel,
            self.parser.current_recipient, self.parser.last_content_delta)

        if self.parser.current_channel == "analysis":
            # Analysis channel -> reasoning_content (no token wrapping needed)
            self.current_channel_state = "analysis"
            return {"reasoning_content": self.parser.last_content_delta}
    ...
    def finalize_request(self) -> Optional[Dict[str, Any]]:
        """
...
class HarmonyAdapter:
    ...
    def openai_to_harmony_tokens(
            ...
            tool_choice: Optional[str] = None) -> List[int]:
        """
        ...
        try:
            ...
            system_instructions = []
            conversation_messages = []  # Store user/assistant/tool messages
            ...
            for msg in openai_messages:
                ...
                if role == "system":
                    # Collect system instructions for developer message
            ...
            # 1. Add system message: reasoning effort, knowledge cutoff, conversation start date, required channels
            # Use explicit reasoning effort if provided, otherwise parse from system messages
            if explicit_reasoning_effort:
                reasoning_effort = explicit_reasoning_effort
            else:
                reasoning_effort = self._parse_reasoning_effort_from_system_messages(
                    system_instructions)
            system_content = self._create_enhanced_system_content(
                reasoning_effort)
            harmony_messages.append(
                Message.from_role_and_content(Role.SYSTEM, system_content))

            # 2. Remove reasoning effort from system instructions
            system_instructions = self._strip_reasoning_effort_from_instructions(
                system_instructions)

            # 3. Developer message with instructions and tools (before conversation)
            if system_instructions or available_tools:
                ...
                if available_tools:
                    ...
                    if tool_descriptions:
                        dev_content = dev_content.with_function_tools(
                            tool_descriptions)

                harmony_messages.append(
                    Message.from_role_and_content(Role.DEVELOPER, dev_content))

            # 4. Process conversation messages (user/assistant/tool)
            ...
            # print("================================================")
            # print("decoded tokens:", self._safe_decode_utf8(tokens, "HARMONY_TOKENS: "))
            # print("================================================")
            return tokens
    ...
    def _apply_harmony_to_openai_mapping(self, analysis_content: str,
        ...
        if analysis_content.strip():
            result["reasoning_content"] = analysis_content.strip()
    ...
    def harmony_output_to_openai(
        ...
        try:
            ...
            clean_tokens = self._strip_stop_tokens(clean_tokens)
            ...
            for msg in harmony_messages:
                ...
                if msg_channel == "analysis":
                    ...
                    if msg_recipient and msg_recipient != 'assistant':
                        # print(f"DEBUG: Found tool call candidate - recipient={msg_recipient}")
                        # Tool call
            ...
            # print(f"DEBUG: Final parsed content - analysis_content='{analysis_content}', tool_calls={tool_calls}, commentary_preambles={commentary_preambles}, final_content='{final_content}'")
    ...
    def get_stop_tokens(self) -> List[int]:
        ...
        # return self.encoding.stop_tokens() # <|end|> <|return|> <|call|>

    def _strip_stop_tokens(self, tokens: List[int]) -> List[int]:
        """
        Strip stop tokens from the end of a token sequence.

        The harmony library's parse_messages_from_completion_tokens expects clean tokens
        without stop tokens AT THE END. Stop tokens in the middle of the sequence
        (like <|end|> and <|call|>) are message separators and should be preserved.
        Only the final stop token should be removed.

        Args:
            tokens: List of token IDs potentially ending with stop tokens

        Returns:
            List of token IDs with stop tokens removed from the end only
        """
        if not tokens:
            return tokens

        stop_tokens = self.encoding.stop_tokens()

        # Make a copy to avoid modifying the original
        clean_tokens = tokens[:]

        # Strip stop tokens from the end only
        while clean_tokens and clean_tokens[-1] in stop_tokens:
            clean_tokens.pop()
    ...
    def stream_harmony_tokens_to_openai_deltas(
            ...
            tokens: List[int],
            available_tools: Optional[List[Dict[str, Any]]] = None,
            tool_choice: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def create_openai_streaming_response(
            ...
            tool_choice: Optional[str] = None) -> List[str]:
        """
        ...
        try:
            ...
            logger.warning("Error creating harmony deltas for request %s: %s",
                           request_id, e)
            logger.debug("Problematic tokens (%d): %s", len(tokens), tokens)
            # Return empty response list to continue processing
            return []
        ...
        is_first_delta = stream_state and not stream_state.first_delta_sent

        # If this is the first delta, send a role-only message first
        if is_first_delta and harmony_deltas:
            # Create role-only first message
            first_delta = DeltaMessage(
                role="assistant",
                content=None,
            ...
            # Mark that we've sent the first delta
            if stream_state:
                stream_state.first_delta_sent = True
        ...
        for harmony_delta in harmony_deltas:
            ...
            if '"tool_calls":[]' in response_json:
                response_json = response_json.replace('"tool_calls":[]',
                                                      '"tool_calls":null')
    ...
    def create_stream_state(
            ...
            request_id: str,
            available_tools: Optional[List[Dict[str, Any]]] = None,
            tool_choice: Optional[str] = None) -> HarmonyStreamState:
        """
    ...
    def _extract_preamble_and_final_content(self,
    ...
    def _convert_assistant_message_from_openai_to_harmony(
            self, assistant_msg: Dict[str, Any], external_tools: Set[str],
    ...
    def _strip_reasoning_effort_from_instructions(
            self, system_instructions: List[str]) -> List[str]:
        """
        Strip reasoning effort specifications from the beginning of the first system instruction only.
        Leaves reasoning effort specifications in the middle of any instruction untouched.
        ...
        Args:
            system_instructions: List of system instruction strings

        Returns:
            List of system instructions with reasoning effort stripped only from the beginning of the first one
        """
        ...
        for i, instruction in enumerate(system_instructions):
            if not instruction:
                continue
            ...
            if i == 0:
                ...
                # Pattern to match reasoning effort specifications at the beginning only
                reasoning_patterns = [
                    r"^reasoning_effort:\s*(high|medium|low)\s*",  # More explicit pattern - check first
                    r"^reasoning:\s*(high|medium|low)\s*"  # Less explicit pattern - check second
                ]

                # Remove reasoning effort specifications from beginning
                for pattern in reasoning_patterns:
                    match = re.match(pattern,
                                     cleaned_instruction,
                                     flags=re.IGNORECASE)
                    if match:
                        # Remove the matched reasoning effort
                        cleaned_instruction = cleaned_instruction[match.end(
                        ):].strip()
                        break

                # Only add non-empty cleaned instructions
                if cleaned_instruction:
                    cleaned_instructions.append(cleaned_instruction)
            else:
                # For all other instructions, keep them as-is (don't strip reasoning effort)
                cleaned_instructions.append(instruction)

        return cleaned_instructions

    def _parse_reasoning_effort_from_system_messages(
            self, system_instructions: List[str]) -> str:
        """
        Parse reasoning effort from the beginning of the first system message content.
        Only detects reasoning effort if it appears at the very beginning of the first system instruction.
        ...
        Args:
            system_instructions: List of system message content strings

        Returns:
            Reasoning effort level ("low", "medium", "high") or default
        """
        ...
        first_instruction = system_instructions[0].strip()
        if not first_instruction:
            logger.debug(
                f"First system instruction is empty, using default reasoning effort: {reasoning_effort}"
            )
            return reasoning_effort

        # Use regex patterns to find reasoning effort specifications at the beginning only
        reasoning_patterns = [
            r"^reasoning_effort:\s*(high|medium|low)",  # Must be at the beginning of string
            r"^reasoning:\s*(high|medium|low)"  # Must be at the beginning of string
        ]

        for pattern in reasoning_patterns:
            match = re.search(pattern, first_instruction, re.IGNORECASE)
            if match:
                reasoning_effort = match.group(1).lower()
                break

        return reasoning_effort

    def _create_enhanced_system_content(self,
    ...
    #     # Add system instructions from OpenAI messages
    #     if system_instructions:
    #         for instruction in system_instructions:
    #             instructions.append(instruction)
    #         instructions.append("")

    #     # Add tool definitions in TypeScript format
    #     if available_tools:
    #         instructions.append("# Tools")
    #         instructions.append("")
    #         instructions.append("## functions")
    #         instructions.append("")
    #         instructions.append("namespace functions {")
    #         instructions.append("")
    ...
    #                 if func_description:
    #                     instructions.append(f"// {func_description}")

    #                 # Generate TypeScript type definition
    #                 type_def = self._generate_typescript_type(func_parameters)
    #                 instructions.append(f"type {func_name} = {type_def};")
    #                 instructions.append("")
    ...
    # def _generate_typescript_type(self, parameters: Dict[str, Any]) -> str:
    #     """Generate TypeScript type definition from OpenAI function parameters."""
    ...
    #     # Generate parameter object type
    #     param_parts = []
    ...
    #         if description:
    #             param_parts.append(f"// {description}")

    #         # Add property with optional marker
    #         optional = "" if prop_name in required else "?"
    #         param_parts.append(f"{prop_name}{optional}: {prop_type},")
    ...
    #     """Convert OpenAI property definition to TypeScript type."""
    ...
    #         return "any"  # Could be expanded to generate nested types
    #     else:
    #         return "any"

    def _parse_tool_call_from_harmony_message(
            self, msg: Message) -> Optional[Dict[str, Any]]:
